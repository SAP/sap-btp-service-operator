...
| `services.cloud.sap.com/allowCrossNamespaceBinding` | `map[string]string` | By default, a service binding for a service instance can be created only in the namespace of the service instance. To allow the creation of service bindings for the service instance in other namespaces, add the following annotation to the service instance: `services.cloud.sap.com/allowCrossNamespaceBinding: "true"`. If this annotation is missing, or is set to `"false"`, requests to create a service binding for the service instance in another namespace are rejected. To restrict cross-namespace binding to specific namespaces, additionally use the `services.cloud.sap.com/allowedNamespaces` annotation. |
| `services.cloud.sap.com/allowedNamespaces` | `map[string]string` | Optional. Restricts cross-namespace binding to specific namespaces. This annotation takes effect only when the service instance is also annotated with `services.cloud.sap.com/allowCrossNamespaceBinding: "true"`. Provide the namespaces in which service bindings for the service instance can be created as a comma-separated list, for example: `services.cloud.sap.com/allowedNamespaces: "namespace-a,namespace-b"`. Requests to create a service binding for the service instance in a namespace that is not on the list are rejected. If this annotation is not set, service bindings for the service instance can be created in any namespace managed by the operator. |
