...
| `services.cloud.sap.com/allowCrossNamespaceBinding` | `map[string]string` | By default, a service instance can be bound only by service bindings in its own namespace. To allow service bindings in other namespaces to bind to the service instance, add the following annotation to the service instance: `services.cloud.sap.com/allowCrossNamespaceBinding: "true"`. If this annotation is missing, or is set to `"false"`, requests to bind the service instance from another namespace are rejected. To restrict cross-namespace binding to specific namespaces, additionally use the `services.cloud.sap.com/allowedNamespaces` annotation. |
| `services.cloud.sap.com/allowedNamespaces` | `map[string]string` | Optional. Restricts cross-namespace binding to specific namespaces. This annotation takes effect only when the service instance is also annotated with `services.cloud.sap.com/allowCrossNamespaceBinding: "true"`. Provide the namespaces that are allowed to bind to the service instance as a comma-separated list, for example: `services.cloud.sap.com/allowedNamespaces: "namespace-a,namespace-b"`. Requests to bind the service instance from a namespace that is not on the list are rejected. If this annotation is not set, service bindings in any namespace managed by the operator can bind to the service instance. |
...
| `serviceInstanceNamespace` | `string` | The namespace of the service instance to bind, if not specified the default is the binding’s namespace. Note that you can bind to a service instance in a different namespace only if the service instance is annotated with `services.cloud.sap.com/allowCrossNamespaceBinding: "true"`. If the service instance is not annotated this way, or if its `services.cloud.sap.com/allowedNamespaces` annotation is set and does not include the binding’s namespace, the binding request is rejected. See [Annotations](#annotations). |
