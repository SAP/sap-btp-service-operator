...
| `services.cloud.sap.com/allowCrossNamespaceBinding` | `map[string]string` | You bind an application to a service instance by creating a service binding. By default, the service binding must be in the same namespace as the service instance. To allow service bindings for the instance in other namespaces, add the following annotation to the service instance: `services.cloud.sap.com/allowCrossNamespaceBinding: "true"`. If this annotation is missing, or is set to `"false"`, requests to create a service binding for the instance in another namespace are rejected. To restrict cross-namespace binding to specific namespaces, additionally use the `services.cloud.sap.com/allowedNamespaces` annotation. |
| `services.cloud.sap.com/allowedNamespaces` | `map[string]string` | Optional. Restricts cross-namespace binding to specific namespaces. This annotation takes effect only when the service instance is also annotated with `services.cloud.sap.com/allowCrossNamespaceBinding: "true"`. Provide a comma-separated list of the namespaces in which service bindings for the instance can be created, for example: `services.cloud.sap.com/allowedNamespaces: "namespace-a,namespace-b"`. Requests from namespaces that are not on the list are rejected. If this annotation is not set, service bindings for the instance can be created in any namespace managed by the operator. |
