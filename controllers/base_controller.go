...
func (r *BaseReconciler) getSMClient(ctx context.Context, object api.SAPBTPResource, btpAccess string) (sm.Client, error) {
	if r.SMClient != nil {
		return r.SMClient(), nil
	}
	log := GetLogger(ctx)

	secret, err := r.SecretResolver.GetSecretForResource(ctx, object.GetNamespace(), secrets.SAPBTPOperatorSecretName, btpAccess)
	if err != nil {
		return nil, err
	}
	...
	if len(cfg.ClientSecret) == 0 {
		tls, err := r.SecretResolver.GetSecretForResource(ctx, object.GetNamespace(), secrets.SAPBTPOperatorTLSSecretName, btpAccess)
